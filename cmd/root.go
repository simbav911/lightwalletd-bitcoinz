...
var rootCmd = &cobra.Command{
	Use:   "lightwalletd",
	Short: "Lightwalletd is a backend service to the BitcoinZ blockchain",
	...
	Run: func(cmd *cobra.Command, args []string) {
		opts := &common.Options{
			...
			BitcoinzConfPath:       viper.GetString("bitcoinz-conf-path"),
		...
		if !opts.Darkside && (opts.RPCUser == "" || opts.RPCPassword == "" || opts.RPCHost == "" || opts.RPCPort == "") {
			filesThatShouldExist = append(filesThatShouldExist, opts.BitcoinzConfPath)
		}
...
func startServer(opts *common.Options) error {
	...
	// Initialize BitcoinZ RPC client. Right now (Jan 2018) this is only for
	...
	if opts.Darkside {
		...
		if opts.RPCUser != "" && opts.RPCPassword != "" && opts.RPCHost != "" && opts.RPCPort != "" {
			...
			rpcClient, err = frontend.NewZRPCFromConf(opts.BitcoinzConfPath)
		}
		if err != nil {
			common.Log.WithFields(logrus.Fields{
				"error": err,
			}).Fatal("setting up RPC connection to zebrad or bitcoinzd")
		}
		// Indirect function for test mocking (so unit tests can talk to stub functions).
		common.RawRequest = rpcClient.RawRequest

		// Ensure that we can communicate with bitcoinzd
		common.FirstRPC()
		...
		if err != nil {
			common.Log.WithFields(logrus.Fields{
				"error": err,
			}).Fatal("getting initial information from zebrad or bitcoinzd")
		}
		...
		if strings.Contains(getLightdInfo.BitcoinZdSubversion, "MagicBean") {
			// The default is zebrad
			common.NodeName = "bitcoinzd"
		}
	}
...
func init() {
	...
	rootCmd.Flags().String("bitcoinz-conf-path", "./bitcoinz.conf", "conf file to pull RPC creds from")
	...
	rootCmd.Flags().Bool("redownload", false, "re-fetch all blocks from zebrad or bitcoinzd; reinitialize local cache files")
	rootCmd.Flags().Bool("nocache", false, "don't maintain a compact blocks disk cache (to reduce storage)")
	rootCmd.Flags().Int("sync-from-height", -1, "re-fetch blocks from zebrad or bitcoinzd, starting at this height")
	...
	viper.BindPFlag("bitcoinz-conf-path", rootCmd.Flags().Lookup("bitcoinz-conf-path"))
	viper.SetDefault("bitcoinz-conf-path", "./bitcoinz.conf")
...
func initConfig() {
	...
	if common.DonationAddress != "" {
		if !strings.HasPrefix(common.DonationAddress, "u") {
			common.Log.Fatal("donation-address must be a BitcoinZ UA address, generate it with a recent wallet")
		}

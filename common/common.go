// Copyright (c) 2019-2020 The BitcoinZ developers
...
type Options struct {
	...
	BitcoinzConfPath       string `json:"bitcoinz_conf,omitempty"`
...
// in unit tests it points to a function to mock RPCs to bitcoinzd.
...
// The following are JSON bitcoinzd rpc requests and replies.
type (
	// bitcoinzd rpc "getblockchaininfo"
	Upgradeinfo struct {
	...
	BitcoinZdRpcReplyGetblockchaininfo struct {
		Chain           string
	...
	// bitcoinzd rpc "getinfo"
	// Note, this rpc should not be depended on in the future (being deprecated).
	BitcoinZdRpcReplyGetinfo struct {
		Build      string
		Subversion string
	}

	// bitcoinzd rpc "getaddresstxids"
	BitcoinZdRpcRequestGetaddresstxids struct {
		Addresses []string `json:"addresses"`
		Start     uint64   `json:"start"`
		End       uint64   `json:"end,omitempty"`
	}

	// bitcoinzd rpc "z_gettreestate"
	BitcoinZdRpcReplyGettreestate struct {
		Height  int
		Hash    string
		Time    uint32
		Sapling struct {
	...
	// bitcoinzd rpc "getrawtransaction txid 1" (1 means verbose), there are
	// many more fields but these are the only ones we current need.
	BitcoinZdRpcReplyGetrawtransaction struct {
		Hex    string
		Height int64
	}

	// bitcoinzd rpc "getaddressbalance"
	BitcoinZdRpcRequestGetaddressbalance struct {
		Addresses []string `json:"addresses"`
	}
	BitcoinZdRpcReplyGetaddressbalance struct {
		Balance int64
	}

	// bitcoinzd rpc "getaddressutxos"
	BitcoinZdRpcRequestGetaddressutxos struct {
		Addresses []string `json:"addresses"`
	}
	BitcoinZdRpcReplyGetaddressutxos struct {
		Address     string
		Txid        string
		OutputIndex int64
		Script      string
	...
	BitcoinZRpcReplyGetblock1 struct {
		Hash  string
		Tx    []string
		Trees struct {
			Sapling struct {
	...
	BitcoinZdRpcReplyGetsubtreebyindex struct {
		Subtrees []Subtree
	}
)

// FirstRPC tests that we can successfully reach bitcoinzd through the RPC
	...
	for {
		...
		if retryCount > 10 {
			Log.WithFields(logrus.Fields{
				"timeouts": retryCount,
			}).Fatal("unable to issue getblockchaininfo RPC call to bitcoinzd or bitcoinzd node")
		}
		Log.WithFields(logrus.Fields{
			"error": err.Error(),
			"retry": retryCount,
...
func GetBlockChainInfo() (*BitcoinZdRpcReplyGetblockchaininfo, error) {
	result, rpcErr := RawRequest("getblockchaininfo", []json.RawMessage{})
	if rpcErr != nil {
		return nil, rpcErr
	}
	var getblockchaininfoReply BitcoinZdRpcReplyGetblockchaininfo
...
func GetLightdInfo() (*walletrpc.LightdInfo, error) {
	...
	var getinfoReply BitcoinZdRpcReplyGetinfo
	...
	vendor := "BitcoinZ LightWalletD"
	if DarksideEnabled {
		vendor = "BitcoinZ DarksideWalletD"
	}
	return &walletrpc.LightdInfo{
		...
		BitcoinZdBuild:             getinfoReply.Build,
		BitcoinZdSubversion:        getinfoReply.Subversion,
...
func getBlockFromRPC(height int) (*walletrpc.CompactBlock, error) {
	...
	if rpcErr != nil {
		// Check to see if we are requesting a height the bitcoinzd doesn't have yet
		if (strings.Split(rpcErr.Error(), ":"))[0] == "-8" {
			return nil, nil
		}
		return nil, fmt.Errorf("error requesting verbose block: %w", rpcErr)
	}
	var block1 BitcoinZRpcReplyGetblock1
...
// the cache, then, if not found, will request the block from bitcoinzd. It returns
...
// ParseRawTransaction converts between the JSON result of a `bitcoinzd`
...
// reinterpret the result of the `getrawtransaction` RPC call. BitcoinZd will
...
// we must map the response from the bitcoinzd RPC API to be representable within
...
func ParseRawTransaction(message json.RawMessage) (*walletrpc.RawTransaction, error) {
	...
	var txinfo BitcoinZdRpcReplyGetrawtransaction

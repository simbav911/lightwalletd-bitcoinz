...
type darksideState struct {
	...
	getAddressUtxos []BitcoinZdRpcReplyGetaddressutxos
...
func darksideRawRequest(method string, params []json.RawMessage) (json.RawMessage, error) {
	...
	case "getblockchaininfo":
		...
		blockchaininfo := &BitcoinZdRpcReplyGetblockchaininfo{
	...
	case "getaddressutxos":
		var req BitcoinZdRpcRequestGetaddressutxos
		err := json.Unmarshal(params[0], &req)
		if err != nil {
			return nil, errors.New("failed to parse getaddressutxos JSON")
		}
		utxosReply := make([]BitcoinZdRpcReplyGetaddressutxos, 0)
	...
	case "z_gettreestate":
		...
		zcashdTreeState := &BitcoinZdRpcReplyGettreestate{}
...
func DarksideAddAddressUtxo(arg BitcoinZdRpcReplyGetaddressutxos) error {
	mutex.Lock()

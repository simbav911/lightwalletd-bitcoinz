...
// GetLatestBlock returns the height and hash of the best chain, according to bitcoinzd.
...
func (s *lwdStreamer) GetTaddressTransactions(addressBlockFilter *walletrpc.TransparentAddressBlockFilter, resp walletrpc.CompactTxStreamer_GetTaddressTransactionsServer) error {
	...
	request := &common.BitcoinZdRpcRequestGetaddresstxids{
...
// See section 3.7 of the BitcoinZ protocol specification. It returns several other useful
...
func (s *lwdStreamer) GetTreeState(ctx context.Context, id *walletrpc.BlockID) (*walletrpc.TreeState, error) {
	...
	// The BitcoinZ z_gettreestate rpc accepts either a block height or block hash
	...
	var gettreestateReply common.BitcoinZdRpcReplyGettreestate
	for {
...
// by the bitcoinzd 'getrawtransaction' RPC.
...
// it gets from its backend bitcoinzd.
...
// SendTransaction forwards raw transaction bytes to a bitcoinzd instance over JSON-RPC
...
func getTaddressBalanceBitcoinZdRpc(addressList []string) (*walletrpc.Balance, error) {
	...
	params := make([]json.RawMessage, 1)
	addrList := &common.BitcoinZdRpcRequestGetaddressbalance{
	...
	if rpcErr != nil {
		...
		return nil, status.Errorf(code,
			"getTaddressBalanceBitcoinZdRpc: getaddressbalance error: %s", rpcErr.Error())
	}
	var balanceReply common.BitcoinZdRpcReplyGetaddressbalance
	err = json.Unmarshal(result, &balanceReply)
	if err != nil {
		return nil, status.Errorf(codes.Unknown,
			"getTaddressBalanceBitcoinZdRpc: failed to unmarshal getaddressbalance reply, error: %s", err.Error())
	}
...
func (s *lwdStreamer) GetTaddressBalance(ctx context.Context, addresses *walletrpc.AddressList) (*walletrpc.Balance, error) {
	...
	r, err := getTaddressBalanceBitcoinZdRpc(addresses.Addresses)
	if err == nil {
		common.Log.Tracef("  return: %+v\n", r)
	}
	return r, err
...
func (s *lwdStreamer) GetTaddressBalanceStream(addresses walletrpc.CompactTxStreamer_GetTaddressBalanceStreamServer) error {
	...
	balance, err := getTaddressBalanceBitcoinZdRpc(addressList)
	if err != nil {
		return err
	}
...
// Last time we pulled a copy of the mempool from bitcoinzd.
...
func getAddressUtxos(arg *walletrpc.GetAddressUtxosArg, f func(*walletrpc.GetAddressUtxosReply) error) error {
	...
	addrList := &common.BitcoinZdRpcRequestGetaddressutxos{
	...
	var utxosReply []common.BitcoinZdRpcReplyGetaddressutxos
...
func (s *lwdStreamer) GetSubtreeRoots(arg *walletrpc.GetSubtreeRootsArg, resp walletrpc.CompactTxStreamer_GetSubtreeRootsServer) error {
	...
	var reply common.BitcoinZdRpcReplyGetsubtreebyindex
...
// and makes them available to present from the mock bitcoinzd's GetBlock rpc.
...
func (s *DarksideStreamer) AddAddressUtxo(ctx context.Context, arg *walletrpc.GetAddressUtxosReply) (*walletrpc.Empty, error) {
	utxosReply := common.BitcoinZdRpcReplyGetaddressutxos{
		Address:     arg.Address,

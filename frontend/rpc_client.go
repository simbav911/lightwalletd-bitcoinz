...
// NewZRPCFromConf reads the bitcoinzd configuration file.
...
// NewZRPCFromFlags gets bitcoinzd rpc connection information from provided flags.
func NewZRPCFromFlags(opts *common.Options) (*rpcclient.Client, error) {
	// Connect to local BitcoinZ RPC server using HTTP POST mode.
	connCfg := &rpcclient.ConnConfig{
		Host:         net.JoinHostPort(opts.RPCHost, opts.RPCPort),
		User:         opts.RPCUser,
		Pass:         opts.RPCPassword,
		HTTPPostMode: true, // BitcoinZ only supports HTTP POST mode
		DisableTLS:   true, // BitcoinZ does not provide TLS by default
	}
	return rpcclient.New(connCfg, nil)
}
